...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--max-fire-rate", type=float, default=0.5,
                   help="Drop latents whose image-side fire rate exceeds this. "
                        "Set to 1.0 to disable filtering (legacy behavior).")
...
def main() -> None:
    ...
    # 3) Always-on latent filter (per paper §4.4; default fire_rate threshold 0.5).
    L = z_val.shape[1]
    fire_rate = (z_val != 0).float().mean(dim=0).cpu().numpy()
    keep = fire_rate <= args.max_fire_rate
    logger.info("always-on filter: keep %d/%d latents (fire_rate≤%.2f)",
                int(keep.sum()), L, args.max_fire_rate)
    ...
    # 4) Top-1 prediction via cosine similarity (chunked to avoid 50k×1000 OOM).
    correct = 0
    bsz = 8192
    ...
    logger.info("val top-1 accuracy (filtered): %.4f", acc)
