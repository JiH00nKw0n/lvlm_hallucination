...
YAML-driven (`configs/real/cc3m.yaml`) + `scripts/real_alpha/run_real_v2.py` 단일 드라이버로 5 method × COCO{recon, retrieval} + ImageNet{recon, zeroshot_raw} = 20 evals + perm 2 + table 1 = 27 step 완료. Wall **3.17 h** (mmap-load 6 s × 5 + 35 min train × 4 + perm 6 min + eval 40 min). Methods: shared / separated / iso_align / group_sparse / **Post-hoc Align (Ours)** = separated ckpt + post-hoc Hungarian on co-firing patterns. 학습 중 0 constraint, 0 hyperparameter — Iso-Energy/Group-Sparse는 학습 중 loss로 정렬을 *강제*하는 반면 우리는 사후에 Hungarian으로 co-firing evidence를 *읽어낼* 뿐.

Training: 10 epochs (CC3M가 COCO Karpathy의 5배 → 30ep 불필요), 27 330 steps × 13 it/s. Eval은 양쪽 모두 OOD (CC3M trained → COCO/ImageNet eval).

**Headline (test set 모두 OOD):** Post-hoc Align이 retrieval + zs 전 지표 1위.
...
- ~~CC3M 기반 real 학습 + Table 1~~ — ✅ k=32 5-method 완료 (`real_exp_cc3m`, §5.6). Post-hoc Align (Ours)가 retrieval + zs 1위 (I→T R@1 10.80, IN zs 27.23%).
