...
class FarSightMitigator(BaseMitigator):
    ...
    def _create_farsight_forward(self, layer_idx: int) -> Callable:
        """
        Create FarSight attention forward.
        ...
        def forward(
            ...
            **kwargs,
            ...
            if mitigator.use_alibi:
                ...
                P_total = P_static.expand(B, self.num_heads, L, L) + P_alibi.expand(B, self.num_heads, L, L)
            else:
                P_total = P_static.expand(B, self.num_heads, L, L)

            # Handle attention mask for padding
            if attention_mask is not None:
                # Derive valid tokens from attention_mask
                if attention_mask.dim() == 2:
                    valid = (attention_mask > 0).to(dtype)
                elif attention_mask.dim() == 4:
                    valid = (attention_mask.squeeze(1).squeeze(1) > -1e8).to(dtype)
                else:
                    valid = torch.ones((B, L), device=device, dtype=dtype)

                valid_cols = valid.view(B, 1, 1, L)
                valid_rows = valid.view(B, 1, L, 1)
                P_total = P_total * valid_cols * valid_rows
    ...
    def setup(self) -> None:
        ...
        for layer_idx in self.target_layers:
            if layer_idx < len(layers):
                ...
                attn.forward = types.MethodType(self._create_farsight_forward(layer_idx), attn)
