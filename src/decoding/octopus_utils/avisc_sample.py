...
from src.decoding.base import get_image_token_indices
...
def sample(
    ...
    def _update_cache_position(model_kwargs: Dict[str, Any], input_ids: torch.Tensor) -> None:
        past_kv = model_kwargs.get("past_key_values")
        if past_kv is None or (hasattr(past_kv, "get_seq_length") and past_kv.get_seq_length() == 0):
            cache_position = torch.arange(input_ids.shape[1], device=input_ids.device)
        else:
            cache_position = torch.tensor([input_ids.shape[1] - 1], device=input_ids.device)
    ...
    while True:
        ...
        _update_cache_position(model_kwargs, input_ids)
        _update_cache_position(model_kwargs_method, input_ids)
        _update_cache_position(model_kwargs_cd, input_ids)
        _update_cache_position(model_kwargs_m3id, input_ids)
        ...
        action = torch.argmax(sf, dim=-1)
        # with open('/home/zlj/AvisC-master/checkpoint/ceshi_output/output_5e6.txt', 'a') as file:
        ...
        if use_avisc:
            ...
            layer_gamma = model_kwargs.get("layer_gamma") or 0.5
            masking_scheme = model_kwargs.get("masking_scheme") or "zeros"
            lamb = model_kwargs.get("lamb") or 100.0
            model_name = model_kwargs.get("model_name") if model_kwargs.get("model_name") is not None else "llava"
            model_type = model_kwargs.get("model_type", model_name)
            model_config = model_kwargs.get("model_config", None)
            ...
            if model_inputs_method.get("past_key_values") is None:
                ...
                if model_type and model_type.lower() == "blip":
                    img_idx = list(range(32))
                else:
                    img_start, img_end = get_image_token_indices(
                        input_ids,
                        model_type=model_type or "llava",
                        config=model_config,
                    )
                    img_idx = list(range(img_start, img_end))
                ...
                #######
                # Thresholding
                att_logits = torch.stack(
                    [attention[i].mean(dim=1)[:, -1, img_idx] for i in top_k_lay_idx], dim=1
                )  # [batch_size, num_layer, seq_len]
                img_att_logits = att_logits.mean(dim=1)

                # except global context token masking
                mask_idx = torch.where(img_att_logits < img_att_logits.mean() + img_att_logits.std() * lamb)[
                    1].unsqueeze(0)
