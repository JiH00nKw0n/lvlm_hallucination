...
class CrepeEvaluator(BaseEvaluator):
    ...
    """
    crepe_scores: Dict[str, List] = Field(default_factory=lambda: defaultdict(list))
    ...
    def _encode_dataset(self, batch_size: int = 128):
        ...
        for samples in dataloader:
            for sample in samples:
                ...
                self.crepe_scores[sample['Negative_type']].append(rank)
    ...
    def evaluate(self, batch_size: int = 128):
        ...
        self._encode_dataset(batch_size=batch_size)
        metrics = {}
        for key, _value in self.crepe_scores.items():
            preds = np.array(_value)
            ...
            for k in [1, 3, 5, 10]:
                sub_metrics[f"image_to_text_R@{k}"] = np.mean(preds < k)
                sub_metrics[f"image_to_text_R@{k}_std"] = np.std(preds < k)
            metrics[key] = sub_metrics
