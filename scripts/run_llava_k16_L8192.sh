#!/usr/bin/env bash
#
# One arm of the LLaVA capacity x sparsity grid: k=16, latent_size=8192.
#
# latent_size is the TOTAL, so the modality-specific (separated / ours) methods
# keep the usual half-and-half split at 4096/side, giving 0.391% active
...
#
#   bash scripts/run_llava_k16_L8192.sh
#   CUDA_VISIBLE_DEVICES=1 bash scripts/run_llava_k16_L8192.sh        # pin a GPU
#   DENSITY=1 bash scripts/run_llava_k16_L8192.sh                     # + per-bin density stats
#   FULL_PERM=0 bash scripts/run_llava_k16_L8192.sh                   # keep the 50k-sample perm
#
...
banner "k=$K latent=$L (4096/side): train + eval -> $ROOT"
