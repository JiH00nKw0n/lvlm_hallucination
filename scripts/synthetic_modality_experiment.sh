...
OUTPUT_ROOT="${OUTPUT_ROOT:-$PROJECT_DIR/outputs/synthetic_modality}"
...
function run_one() {
    ...
    local log_file
    log_file="$PROJECT_DIR/.log/syn_mod_${condition}_fd${feature_dim}_${ts}.log"

    echo "======================================================"
    echo "Condition=${condition} | fd=${feature_dim} | rep=${REPRESENTATION_DIM} | latent=${LATENT_SIZE} | k=${K} | s=${SPARSITY}"
...
with open(summary_md, "w", encoding="utf-8") as f:
    f.write("# Synthetic Modality Comparison Summary\n\n")
...
with open(compare_md, "w", encoding="utf-8") as f:
    f.write("# Unimodal vs Multimodal Comparison\n\n")
    if compare_rows:
