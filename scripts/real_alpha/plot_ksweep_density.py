...
ALIVE_THR = 0.001
ALPHA = 0.2
# Cosine distance d = 1 - cos. Hungarian-matched pairs concentrate in [0, 1.3].
...
def plot_density_subplot(ax, C_matched, dist_matched, title: str):
    ...
    ax.set_xlabel(r"cosine distance $1-\cos(\hat\phi_{i},\hat\psi_{j})$",
...
def main():
    ...
    # Single row: 4 panels side by side, share y so density scale is visible.
    fig, axes = plt.subplots(1, n, figsize=(1.8 * n, 1.6), sharex=True, sharey=True)
    if n == 1:
        axes = [axes]
    ...
    # Only the leftmost panel shows the y-label; all panels keep xlabel.
    for ax in axes[1:]:
        ax.set_ylabel("")
    axes[0].set_ylabel("density", fontsize=8, labelpad=2)
    ...
    plt.subplots_adjust(left=0.06, right=0.99, bottom=0.30, top=0.88, wspace=0.18)
