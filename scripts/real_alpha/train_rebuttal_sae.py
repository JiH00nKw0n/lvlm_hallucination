"""Train SAEs for the rebuttal experiments, with seed-controlled initialization.

Copy of ``train_real_sae.py`` with one behavioral change: the RNG is seeded
*before* the model is constructed. HF Trainer calls ``set_seed()`` inside its
own ``__init__``, which runs after ``build_*_trainer()`` has already built the
module, so in the original script ``--seed`` only reshuffles the data order and
the initial weights come from the uncontrolled global RNG.

That distinction does not matter for the paper's results, which is why the
original is left untouched. It matters here: the same-modality control compares
two independently trained runs and claims the only difference between them is
the seed, so the initialization has to be seed-controlled and reproducible. The
initial weight hash is written to ``init_signature.json`` so the claim can be
checked after the fact.

Usage:
    # Setting F (paper Figure 2 operating point), run 1 of 3
    python scripts/real_alpha/train_rebuttal_sae.py \
        --variant two_sae --dataset coco --cache-dir cache/clip_b32_coco \
        --output-dir outputs/rebuttal_models/coco_k8_r1 \
        --latent 8192 --k 8 --epochs 30 --batch-size 1024 \
        --lr 5e-4 --warmup-ratio 0.05 --weight-decay 1e-5 --seed 1

    # Setting T (paper Table 1 operating point), run 1 of 3
    python scripts/real_alpha/train_rebuttal_sae.py \
        --variant two_sae --dataset cc3m --cache-dir cache/clip_b32_cc3m \
        --output-dir outputs/rebuttal_models/cc3m_k32_r1 \
        --latent 8192 --k 32 --epochs 10 --batch-size 1024 \
        --lr 5e-4 --warmup-ratio 0.05 --weight-decay 1e-5 --seed 1
"""
...
def _weight_signature(model) -> str:
    """Short hash of the current parameters, used to prove two runs started
...
def main() -> None:
    ...
    # The one change from train_real_sae.py: seed before the model is built.
    ...
    init_sig = _weight_signature(trainer.model)
    logger.info("init weight signature (seed=%d): %s", args.seed, init_sig)
    with open(output_dir / "init_signature.json", "w") as f:
        json.dump({"seed": args.seed, "init_weight_sha256_16": init_sig}, f, indent=2)
