...
OUTPUT_ROOT="${OUTPUT_ROOT:-$PROJECT_DIR/outputs/synthetic_importance}"
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --conditions) CONDITIONS_STR="$2"; shift 2 ;;
        --importance-decays) IMPORTANCE_DECAYS_STR="$2"; shift 2 ;;
        --importance-targets) IMPORTANCE_TARGETS_STR="$2"; shift 2 ;;
...
function run_one() {
    ...
    local log_file
    log_file="$PROJECT_DIR/.log/syn_imp_${condition}_d${decay}_t${target}_${ts}.log"

    echo "======================================================"
    echo "Condition=${condition} | decay=${decay} | target=${target}"
    echo "fd=${FEATURE_DIM} | rep=${REPRESENTATION_DIM} | latent=${LATENT_SIZE} | k=${K}"
    echo "Log: ${log_file}"
    echo "======================================================"
...
with open(summary_md, "w", encoding="utf-8") as f:
    f.write("# Importance Hierarchy Experiment Summary\n\n")
