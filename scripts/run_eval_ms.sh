...
OUT_ROOT=${OUT_ROOT:-$ROOT/ms_${DATASET}_${SPLIT}}
LOG_DIR=${LOG_DIR:-.log}
mkdir -p "$LOG_DIR"
...
global_start=$(date +%s)
log "BEGIN MS eval dataset=$DATASET split=$SPLIT ext=$EXT_CACHE out=$OUT_ROOT"
...
for variant in "${VARIANTS[@]}"; do
  ...
  if [[ -f "$out/ms_summary.json" ]]; then
    log "  SKIP — already done: $out/ms_summary.json"; continue
  fi
  ...
  python scripts/real_alpha/eval_ms.py \
