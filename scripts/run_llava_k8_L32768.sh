#!/usr/bin/env bash
#
# One arm of the LLaVA capacity x sparsity grid: k=8, latent_size=32768.
#
# latent_size is the TOTAL, so the modality-specific (separated / ours) methods
# keep the usual half-and-half split at 16384/side, giving 0.049% active
...
#   CUDA_VISIBLE_DEVICES=1 bash scripts/run_llava_k8_L32768.sh        # pin a GPU
#   DENSITY=1 bash scripts/run_llava_k8_L32768.sh                     # + per-bin density stats
#   FULL_PERM=0 bash scripts/run_llava_k8_L32768.sh                   # keep the 50k-sample perm
#
