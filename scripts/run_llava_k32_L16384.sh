#!/usr/bin/env bash
#
# One arm of the LLaVA capacity x sparsity grid: k=32, latent_size=16384.
#
# latent_size is the TOTAL, so the modality-specific (separated / ours) methods
# keep the usual half-and-half split at 8192/side, giving 0.391% active
...
#
#   bash scripts/run_llava_k32_L16384.sh
#   CUDA_VISIBLE_DEVICES=1 bash scripts/run_llava_k32_L16384.sh        # pin a GPU
#   DENSITY=1 bash scripts/run_llava_k32_L16384.sh                     # + per-bin density stats
#   FULL_PERM=0 bash scripts/run_llava_k32_L16384.sh                   # keep the 50k-sample perm
#
