...
def process_image_pass(
    ...
    """Run image-only forward pass. Returns (feature_counts, n_img_tokens) or None on failure."""
...
def process_text_pass(
    ...
    """Run text-only forward pass (no pixel_values). Returns (feature_counts, n_txt_tokens) or None."""
...
def plot_histogram(
    ratio: Tensor, alive_mask: Tensor, output_dir: str, sae_path: str, k: int, bin_width: float = 0.05,
) -> str:
    ...
    ax.hist(alive_ratios, bins=n_bins, range=(0.0, 1.0), color="#4C72B0", edgecolor="white", linewidth=0.3)
    ...
    n_text = (alive_ratios < 0.2).sum()
    n_shared = ((alive_ratios >= 0.2) & (alive_ratios <= 0.8)).sum()
    n_image = (alive_ratios > 0.8).sum()

    ax.set_xlabel("Modality Ratio (0=Text-only, 1=Image-only)", fontsize=12)
    ax.set_ylabel("Number of Features", fontsize=12)
    ax.set_title(f"SAE Feature Modality Distribution\n{sae_name} (k={k}, alive={len(alive_ratios):,})", fontsize=13)
    ...
    filepath = os.path.join(output_dir, f"MODALITY_SPLIT_{sae_name}_k{k}.png")
...
def main():
    ...
    plot_histogram(ratio_tensor, alive_tensor, args.output_dir, args.sae_path, args.k, args.bin_width)
